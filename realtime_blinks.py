...
class Visualization(QMainWindow):
    def __init__(self):
        ...
        self.plot_widget_blinks.setMenuEnabled(enableMenu=False)

        self.plot_widget_blinks.getAxis("bottom").setStyle(showValues=False)
        self.plot_widget_blinks.getAxis("top").setStyle(showValues=False)
        self.plot_widget_blinks.getAxis("right").setStyle(showValues=False)
        self.plot_widget_blinks.getAxis("left").setStyle(showValues=False)

        self.plot_widget_blinks.getAxis("bottom").setTicks([])
        ...
        self.plot_curve_blinks = LiveLinePlot(
            pen=pg.mkPen(color=colors.PURPLE_255, width=3)
        )

        self.plot_widget_blinks.addItem(self.plot_curve_blinks)

        self.data_connector_blinks = DataConnector(
            self.plot_curve_blinks, max_points=300, update_rate=200
        )
